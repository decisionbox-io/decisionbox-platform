...
func TestRecommendationsRules_ContainsExpectedRules(t *testing.T) {
	...
	assertContainsAll(t, "RecommendationsRules", text, []substringChecker{
		...
		{"rule R header — related_insight_ids must be UUIDs", "RELATED_INSIGHT_IDS MUST BE VERBATIM UUIDs"},
		{"rule R names UUID shape", "36-character UUID"},
		{"rule R forbids slug shape — issue #237 regression anchor", "ecommerce-conversion-funnel:critical:inconsistent-product-category"},
		{"rule R tells the LLM to omit rather than guess", "OMIT that recommendation"},
