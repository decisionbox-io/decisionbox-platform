...
This filter runs on **both** the schema-index pass and discovery, so an out-of-scope table is dropped before its schema is pulled, blurbed, and embedded — the index (and therefore everything that reads it: Ask, pack generation, discovery) is built over the filtered set, not the full catalog.
