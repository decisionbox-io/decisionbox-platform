...
| Variable | Source | Description | Example |
|----------|--------|-------------|---------|
| `{{SCHEMA_INFO}}` | Warehouse schema discovery | Level-0 catalog only — one line per table (name, column count, row count, keyword hints, joins). The agent fetches per-table column lists and sample rows on demand via `lookup_schema` and `search_tables` actions during exploration. See docs/reference/prompt-variables.md. | `analytics_data.users \| 14c \| 50K rows \| users, accounts \| -> sessions` |
