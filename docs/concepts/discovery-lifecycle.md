...
- A run whose novelty cannot be measured falls back to `--min-steps` rather than refusing every completion until the cap. A degraded index should not turn every run into a maximum-length one against a source metered per request. Three cases count: no vector index is wired at all (the rule is never armed), the index errors on read, and the index accepts no writes. The last one matters because an empty search result means opposite things depending on the index's state — before anything has been offered to it, empty is expected and the step genuinely broke new ground; after steps have been offered and none kept, empty means the index is not storing anything and says nothing about the step at all.
