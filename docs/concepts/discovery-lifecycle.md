...
- A run whose novelty cannot be measured falls back to `--min-steps` rather than refusing every completion until the cap. A degraded index should not turn every run into a maximum-length one against a source metered per request. Three cases count: no vector index is wired at all (the rule is never armed), the index errors on read, and the index accepts no writes — the last matters because an empty search result from an index that is storing nothing looks exactly like "nothing like this has been asked before", and only one of those is evidence.
