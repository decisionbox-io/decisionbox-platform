...
- A run whose novelty cannot be measured falls back to `--min-steps` rather than refusing every completion until the cap. A degraded index should not turn every run into a maximum-length one against a source metered per request. Three cases reach that fallback: no vector index is wired at all (the rule is never armed), the index has been offered steps and kept none (its write path is failing, so every search comes back empty and none of its answers are evidence), and several steps in a row could not be judged (its reads are failing).
