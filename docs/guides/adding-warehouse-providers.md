...
| `ValidateSQL` | Compile a SQL statement without executing it | Route through the warehouse's native compile-only path: `EXPLAIN <sql>` on Postgres-family / Databricks / Redshift, `EXPLAIN USING TEXT <sql>` on Snowflake, `QueryConfig.DryRun = true` on BigQuery, `SET NOEXEC ON; <sql>; SET NOEXEC OFF;` on SQL Server. Reject empty / whitespace SQL at the boundary and propagate the warehouse's own error verbatim. Must NOT execute the statement — read-only enforcement remains `ValidateReadOnly`'s concern. |
