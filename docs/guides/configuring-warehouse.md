...
**Service Account Key** — For cross-cloud or local access.
...
Also supports Workload Identity Federation credential configs (generated by `gcloud iam workload-identity-pools create-cred-config`).
The GCP SDK auto-detects the format.
