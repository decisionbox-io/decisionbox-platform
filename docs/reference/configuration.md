...
| `AGENT_FORWARD_ENV` | _(unset)_ | Comma-separated list of **additional** API-process variable names to forward to spawned agent containers, on top of the built-in set. Only the container-spawning runners (Kubernetes Jobs, Docker) need this — a `RUNNER_MODE=subprocess` agent inherits the API environment wholesale. Use it for a gate belonging to a plugin that ships separately, whose name this repo does not know: a gate set on the API but never forwarded fails **silently**, because the plugin still loads and hands back its no-op implementation, so the run completes looking healthy while doing none of what was enabled. Names, not values — each is read back out of the API's own environment, and one that is unset there is not forwarded at all (rather than forwarded empty, which many gates read as a deliberate "off"). Names already in the built-in set, and names repeated within the list, are ignored: Kubernetes rejects a pod spec carrying the same env name twice. Example: `AGENT_FORWARD_ENV=MY_PLUGIN_ENABLED,MY_OTHER_GATE`. |
