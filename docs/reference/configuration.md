...
| `EXPLORATION_MAX_OUTPUT_TOKENS` | `16384` | Per-step output-token ceiling for the exploration LLM call. Large enough for a reasoning model's thinking block plus the action JSON, yet bounded so the reservation can't overflow the model's context window as the conversation grows. The effective budget is the smaller of this and the model's catalogued output cap. |
