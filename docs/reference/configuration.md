...
| `ANALYSIS_REPAIR_MAX_ROUNDS` | `2` | How many corrective re-prompts an individual insight gets when a quantifier claim it declared is contradicted by the rows of the exploration step it cited. The analysis model declares what each *only / every / largest / Nth / has N values* statement rests on, the agent evaluates that predicate in Go over the step's full rows, and a claim the rows refute is handed back with the reason — including the rows that refuted it — for one insight to be rewritten. Every check is re-run on the rewrite: a round is discarded if it contradicts more of its evidence than the original, or if it keeps the sentence and merely deletes the declaration. The rewrite may not change the insight's `source_steps`. When the rounds are spent and the claim is still refuted, **the sentence is removed and the rest of the finding is kept** — the document is never dropped for one bad sentence; a claim that is the finding's own headline has no sentence to remove, so it ships recorded as unrepaired with its verdict attached. Set to `0` to disable the re-prompting, which leaves the two repairs that need no model: a count the agent computed is substituted directly, and a still-refuted sentence is removed. Costs nothing on the happy path — repair only runs on an insight whose own cited evidence contradicts it, and the per-area counters `insights_repaired`, `insights_claims_dropped`, `insights_unrepaired` and `analysis_repair_rounds` are all omitted when it never fires. |
