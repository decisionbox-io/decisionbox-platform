...
| `--min-steps` | No | `0` | Minimum exploration steps before the agent accepts a `done` signal from the LLM. Early `done` signals are rejected (recorded as `complete_rejected`) and exploration continues. `0` disables the floor. Use on reasoning models (Qwen3, DeepSeek-R1, GPT-OSS) that terminate too early. **Not applied on a run that can query a cube-shaped datasource** — those stop on whether exploration is still finding anything new, since a step count says nothing about coverage of a source with no tables. See [Discovery lifecycle](../concepts/discovery-lifecycle.md). |
