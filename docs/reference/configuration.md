...
| `ANALYSIS_REPAIR_MAX_ROUNDS` | `2` | How many corrective re-prompts an individual insight gets when a quantifier claim it declared is contradicted by the rows of the exploration step it cited. The analysis model declares what each *only / every / largest / Nth / has N values* statement rests on, the agent evaluates that predicate in Go over the step's full rows, and a claim the rows refute is handed back with the reason — including the rows that refuted it — for one insight to be rewritten. Every check is re-run on the rewrite, and a round is discarded unless every formerly-refuted claim it still declares now **holds**: it is discarded if it contradicts more of its evidence than the original, if it keeps the sentence and merely deletes the declaration, or if the claim survives but only evaluates *undecidable* — pointing a claim at an uncited step or naming a column the rows do not carry makes the evaluator decline rather than refuse, which exempts the claim from checking instead of making it true. The rewrite may not change the insight's `source_steps`. When the rounds are spent and the claim is still refuted, **the sentence is removed and the rest of the finding is kept** — the document is never dropped for one bad sentence; a claim that is the finding's own headline has no sentence to remove, so it ships recorded as unrepaired with its verdict attached. A declaration the evaluator cannot read is never reported as a failure: a cardinality claim with no positive `count`, a monotonic claim whose `trend` is not exactly `increasing` or `decreasing`, a rank claim whose `order` is not `asc` or `desc`, a filter naming a column the rows do not carry, and a top-N over a capped result whose rows are not in the claimed order all settle as *undecidable*. Absent metadata read as an assertion would refute a sound sentence and send it through repair over nothing the rows said. A refuted claim that leaves undeclared and was never a sentence in the body is recorded as **withdrawn**, not repaired — the declaration was about nothing the document said, so nothing was corrected and the count of repairs stays honest. Set to `0` to disable the re-prompting, which leaves the two repairs that need no model: a count the agent computed is substituted directly, and a still-refuted sentence is removed. Costs nothing on the happy path — repair only runs on an insight whose own cited evidence contradicts it, and the per-area counters `insights_repaired`, `insights_claims_dropped`, `insights_unrepaired` and `analysis_repair_rounds` are all omitted when it never fires. |
| `DISCOVERY_TRACE` | _unset_ | Emits the evidence chain behind each shipped claim as structured log lines, for diagnosing a finding that turns out to be wrong. Four events: `query` (per executed step — the SQL, row count, fidelity caveats, and how many of the result's rows the digest reproduces verbatim), `exposure` (per analysis area and step, how much of that step's result the prompt actually shows — a step that returned 150 rows and shows 10 can support "the top 10 are …" and not "there are 10 of them", and the digest reads the same either way), `claim` (every declared quantifier claim with the verdict Go reached) and `insight` (one line per shipped finding naming the steps it cited and what repair did to it). Anything other than a truthy value leaves it off, and with it off none of this work is done — the events are built only when the gate is set, so an operator who has never heard of this variable pays nothing. Intended for a diagnostic run, not for production: `query` and `exposure` emit one line per step and per area-step pair. |
...
| `--min-steps` | No | `0` | Minimum exploration steps before the agent accepts a `done` signal from the LLM. Early `done` signals are rejected (recorded as `complete_rejected`) and exploration continues. `0` disables the floor. Use on reasoning models (Qwen3, DeepSeek-R1, GPT-OSS) that terminate too early. **Always applied**, including on a run that can query a cube-shaped datasource: such a run also has tables (a source that cannot anchor may not run discovery alone), so the floor stays a valid coverage proxy. On those runs a no-new-signal rule is added *past* the floor and can only extend a run, never shorten it. See [Discovery lifecycle](../concepts/discovery-lifecycle.md). |
