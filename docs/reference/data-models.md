...
| `fix_attempts` | int | Number of LLM fix calls made for this step's query (0 if it ran cleanly on the first try). Equals `len(fix_history)`. |
| `fixed` | bool | True if the query was auto-fixed after a SQL error |
| `fix_history` | FixAttempt[] | Per-attempt fix log; omitted (or empty) when no fix was needed. See [FixAttempt](#fixattempt). |
...
chronological order. Every entry — including unsuccessful proposals whose
SQL still failed on the next warehouse round-trip — is recorded so
downstream tooling has visibility into the full repair trajectory, not
just the last call.
...
| `response_out` | string | Raw LLM response text — before SQL extraction. |
| `sql_before` | string | The broken SQL handed to the fixer. |
| `sql_after` | string | The SQL the fixer proposed. Success is implicit: if attempt N's `sql_after` equals attempt N+1's `sql_before`, the proposal still failed; if it's the final attempt and the step has no `error`, it succeeded. |
