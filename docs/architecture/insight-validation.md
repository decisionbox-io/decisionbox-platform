...
**Recommendation-eligibility verdicts.** A separate per-project setting (`Project.RecommendationVerdicts`, Settings → Advanced → *Recommendation eligibility*) controls which validation verdicts qualify an insight for the Phase 5 recommender (see [Pipeline placement](#pipeline-placement)). It is a subset of the five user-facing per-claim verdicts (`confirmed`, `supported`, `partial`, `unverifiable`, `rejected`); unset/empty resolves to the default `{confirmed, supported}`, so legacy projects are byte-identical. Both services resolve it via `EffectiveRecommendationVerdicts()` (agent + API `models.Project`, kept in sync), which sanitises the stored strings through `validation.ParseStatuses` and falls back to `validation.DefaultRecommendationVerdicts()`. Typical use: include `partial`/`unverifiable` when the warehouse can't self-verify a claim (e.g. the verifier's temp-table queries are denied) but the analysis is trusted. The two fail-open states above (`validation_disabled` / missing `Validation`) remain eligible regardless of this setting.
