...
test-go: ## Run Go unit tests (auto-discovers all provider and domain pack modules)
	cd libs/go-common && go test ./...
	@for dir in domain-packs/*/go/; do echo "Testing $$dir..." && (cd $$dir && go test ./...) || exit 1; done
	@for dir in providers/llm/*/; do [ "$$(basename $$dir)" = "all" ] && continue; echo "Testing $$dir..." && (cd $$dir && go test ./...) || exit 1; done
	@for dir in providers/secrets/*/; do [ "$$(basename $$dir)" = "all" ] && continue; echo "Testing $$dir..." && (cd $$dir && go test ./...) || exit 1; done
	@for dir in providers/warehouse/*/; do [ "$$(basename $$dir)" = "all" ] && continue; echo "Testing $$dir..." && (cd $$dir && go test ./...) || exit 1; done
...
lint-go: ## Run golangci-lint on all Go modules (install: https://golangci-lint.run/welcome/install/)
	...
	@for dir in domain-packs/*/go/; do echo "Linting $$dir..." && (cd $$dir && golangci-lint run ./...) || exit 1; done
