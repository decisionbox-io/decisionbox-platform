...
func TestRegistered(t *testing.T) {
	...
	if meta.Dialect != "Snowflake SQL" {
		t.Errorf("expected dialect 'Snowflake SQL', got %q", meta.Dialect)
	}
...
func TestValidateSQL_PrefixesExplainUsingText(t *testing.T) {
	mock := &mockSFClient{
		queryFunc: func(ctx context.Context, query string, args ...interface{}) (*sql.Rows, error) {
	...
	if !strings.HasPrefix(mock.lastQuery, "EXPLAIN USING TEXT ") {
		t.Errorf("query should be prefixed with EXPLAIN USING TEXT, got: %q", mock.lastQuery)
	}
...
func TestValidateSQL_WrapsErrorWithPrefix(t *testing.T) {
	mock := &mockSFClient{
		queryFunc: func(ctx context.Context, query string, args ...interface{}) (*sql.Rows, error) {
			return nil, fmt.Errorf("SQL compilation error: syntax error line 1 at position 0 unexpected 'SELEC'")
		},
	}
	p := &SnowflakeProvider{client: mock, timeout: 5 * time.Second}
	...
	if !strings.Contains(err.Error(), "snowflake: validate SQL") {
		t.Errorf("error should carry the validate-SQL prefix, got: %v", err)
	}
	if !strings.Contains(err.Error(), "SQL compilation error") {
