...
prompt_secret() {
  ...
  GO_BACK=false
  local back_hint="${DIM}(back)${NC}"
  read -srp "$(echo -e "${CYAN}?${NC} ${prompt_text} ${back_hint}: ")" value
  echo ""
  if [[ "$value" == "back" ]]; then GO_BACK=true; return 1; fi
  while [[ -z "$value" ]]; do
    err "This field is required."
...
do_step_4_authentication_config() {
  ...
  if [[ "$AUTH_ENABLED" == "true" ]]; then
    ...
    prompt_secret AUTH_CLIENT_SECRET "OIDC client secret" || return 1
...
EOF
  ...
  if [[ "$AUTH_ENABLED" == "true" ]]; then
    ...
    # Preserve existing NEXTAUTH_SECRET on resume to avoid invalidating sessions
    if [[ -f "$DASH_AUTH_VALUES" ]]; then
      EXISTING_SECRET=$(grep "NEXTAUTH_SECRET" "$DASH_AUTH_VALUES" 2>/dev/null | sed 's/.*: *"//;s/"//' || echo "")
    fi
    NEXTAUTH_SECRET="${EXISTING_SECRET:-$(openssl rand -base64 32)}"
...
EOF
    chmod 600 "$DASH_AUTH_VALUES"
    ok "Generated ${DASH_AUTH_VALUES} (mode 600)"
  fi
