...
- **Schema indexing + retrieval** — Projects now build a persistent, searchable schema index at creation time (blurbs + embeddings in Qdrant). Every agent prompt that previously embedded the full warehouse schema now composes a **Level 0 catalog of every table** (always included) **+ Level 1 top-K retrieved details** for the current task — unblocking discovery on ERP-scale warehouses (2K+ tables) while staying identical in behaviour on small ones. Project creation grows an "indexing" phase visible as a progress panel; users can close the tab and return later. `/ask` also gates on the index being ready (same 409 contract as `POST /discover`). Re-indexing is user-triggered from Project Settings → Advanced when the warehouse schema changes. Qdrant and an embedding provider are now required. Defaults from the spike against a real 2K-table ERP: Bedrock `qwen.qwen3-32b-v1:0` for blurbs × OpenAI `text-embedding-3-large` for embeddings (perfect MRR, ~$0.60 / 2K tables, ~6 min wall-clock at 8 parallel workers). Reasoning-class models (DeepSeek R1, o1/o3/o4-mini, Claude extended-thinking) are rejected at save time — their `<think>` channel doesn't carry user-visible text through Converse/Chat. New endpoints `GET /api/v1/projects/{id}/schema-index/status`, `POST /schema-index/retry`, `POST /reindex`. New template variables `{{SCHEMA_CATALOG}}` + `{{SCHEMA_RETRIEVED}}` replace the old `{{SCHEMA_INFO}}` JSON dump (kept as a backward-compat alias so custom user templates don't break). Tool-use plumbing on the LLM interface (Claude Messages + OpenAI function calling + Bedrock Converse) with integration tests against real APIs — the `inspect_table` Level-2 executor ships and is unit-tested, but protocol-level wiring into the exploration engine's action parser is a follow-up (the top-K Level-1 block already covers the common case; Level 2 is for long-tail "the model wants to look at a table that didn't make Level 1"). Crash-recovery sweep on API boot resets projects stuck in "indexing" > 2h (previous API crash left them orphaned). New `docs/guides/schema-indexing.md` covers the lifecycle, defaults, cost envelope, and re-index triggers.
