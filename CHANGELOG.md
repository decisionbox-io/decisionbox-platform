...
- **OIDC authentication and RBAC** — Authenticate users via any OIDC-compliant identity provider (Auth0, Okta, Entra ID, Google Workspace, Keycloak, AWS Cognito). Role-based access control with three roles (viewer, member, admin) enforced per API endpoint. Org-scoped data isolation via JWT claims. Configurable claim mapping for different IdP conventions. NoAuth mode (default) for development and internal deployments. Keycloak testcontainer integration tests.
