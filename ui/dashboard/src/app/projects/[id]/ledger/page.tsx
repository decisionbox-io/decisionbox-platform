...
import { Loader, Select, Button, Group, Stack, Text, Card, Badge, Progress, Tooltip } from '@mantine/core';
...
export default function LedgerPage() {
  ...
  if (loading) {
    return <Shell><Group justify="center" p="xl"><Loader /></Group></Shell>;
  }
  ...
  return (
    <Shell>
      <Stack gap="lg">
        ...
        {/* Findings */}
        <div>
          <SectionHeader title="Findings" count={ledger.findings.length} />
        ...
        {/* Convergence trend */}
        {ledger.convergence.length > 1 && (
          <Card withBorder padding="md" radius="md">
            <Text size="sm" fw={600} mb="xs">Convergence — new findings per run</Text>
            <Stack gap={6}>
              {ledger.convergence.slice(-8).map((c) => (
                <Group key={c.run_id} gap="sm" wrap="nowrap">
                  <Text size="xs" c="dimmed" w={90}>{new Date(c.date).toLocaleDateString()}</Text>
                  <Progress value={Math.round(c.marginal_ratio * 100)} w={200} size="sm" />
                  <Text size="xs" c="dimmed">{c.new_findings} new / {c.total_findings}</Text>
                </Group>
              ))}
            </Stack>
          </Card>
