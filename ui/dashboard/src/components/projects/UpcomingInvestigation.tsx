...
export function UpcomingInvestigation({ projectId }: { projectId: string }) {
  ...
  const collapsedSummary = [
    pending.length > 0 ? t('changesCount', { count: pending.length }) : null,
    tasks.length > 0 ? t('threadsCount', { count: tasks.length }) : null,
  ].filter(Boolean).join(' · ');
      ...
      <Group justify="space-between" align="center" wrap="nowrap">
        <UnstyledButton onClick={toggleCollapsed} aria-expanded={!collapsed} style={{ flex: 1, minWidth: 0 }}>
          <Group gap="xs" wrap="nowrap">
            ...
            <Text fw={600} size="md">{t('whatHappensNext')}</Text>
        ...
        <Anchor component={Link} href={ledgerHref} size="sm" style={{ flexShrink: 0 }}>
          <Group gap={4} wrap="nowrap">{t('openLedger')} <IconArrowRight size={14} /></Group>
        </Anchor>
      </Group>

      <Collapse in={!collapsed}>
        <Text size="xs" c="dimmed" mt="xs" mb="md">
          {t('carriedIntoNextRun')}
        </Text>
        ...
        {pending.length > 0 && (
          <Group gap="xs" wrap="nowrap" mb={shown.length ? 'sm' : 0}>
            ...
            <Text size="sm">
              {t('proposedPlaybookChanges', { count: pending.length })}
            </Text>
          </Group>
        )}
        ...
        {moreTasks > 0 && (
          <Text size="xs" c="dimmed" mt="xs">
            {t('moreOpenThreadsInLedger', { count: moreTasks })}
          </Text>
        )}
      </Collapse>
    </Card>
