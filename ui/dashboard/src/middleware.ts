...
// bodyTimeout to 5 minutes. That cap is too tight for any synchronous
// API endpoint that occasionally does multi-minute work. The
// dispatcher below raises both caps to 20 minutes as a safety belt;
// individual endpoints should still strive to keep their inbound
// request fast and hand long work off asynchronously (the
// pack-generation handler is the canonical example — it responds 202
// and the dashboard polls project state). Browser-side fetch has no
// comparable timeout, so raising the proxy is sufficient end-to-end.
